...
struct event_data {
    ...
    int (*e_fn)(/*VARARGS*/); /* callback function */
...
event_timeout(long usecs,
    ...
    printf("event_timeout: install one\n");
    ...
    e->e_arg = arg;
    e->e_type = EVENT_TIME;
    t0.tv_sec = usecs / 1000000;
    t0.tv_usec = usecs % 1000000 * 1000;
 ...
 */
static int
event_delete(struct event_data **firstp, int (*fn)(),
        void *arg) {
 ...
 */
int
event_timeout_delete(int (*fn)(void*),
        void *arg) {
...
printfd(char *str) {
    ...
    for (e = ee; e; e = e->e_next) {
        if (e->e_type == EVENT_FD) {
            printf("%s: Have event for fd %d fn 0x%x arg 0x%x\n", str, e->e_fd, (unsigned) e->e_fn, (unsigned) e->e_arg);
            return 0;
        } else {
            printf("event at 0x%x is not FD event\n");
        }
    }
...
event_fd(int fd, int (*fn)(int, void*), void *arg, char *str) {
    ...
    printf("Created C event for fd %d fn 0x%x arg 0x%x\n", fd, (unsigned) fn, (unsigned) arg);
    printfd("after");
    return 0;
}
...
#endif /* DEBUG */
            printf("eventloop: timeout : %s[arg: %x]\n",
                    e->e_string, (int) e->e_arg);
